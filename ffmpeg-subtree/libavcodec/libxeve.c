...
static int get_conf(AVCodecContext *avctx, XEVE_CDSC *cdsc)
{
    ...
    if (avctx->framerate.num > 0) {
        ...
        cdsc->param.fps = av_q2d(avctx->framerate);
    }
        ...
        if (ret == XEVE_OK_OUT_NOT_AVAILABLE) { // Return OK but picture is not available yet
            *got_packet = 0;
            ...
            if (xectx->stat.write > 0) {
                ...
                if ((int)(xectx->cdsc.param.fps * 1000) != ((int)xectx->cdsc.param.fps) * 1000)
                  avpkt->time_base.num = 1001;
                else
                  avpkt->time_base.num = 1000;

                avpkt->time_base.den = ((int)xectx->cdsc.param.fps) * 1000;
...
static const AVOption libxeve_options[] = {
    ...
    { "rc-type", "Rate control mode", OFFSET(rc_mode), AV_OPT_TYPE_INT, { .i64 = XEVE_RC_CQP }, XEVE_RC_CQP,  XEVE_RC_CRF, VE, .unit = "rc-type" },
